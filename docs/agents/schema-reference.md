...
Every item carries these fields regardless of type:
...
Holds an array of wikilinks to related items in your knowledge base.
...
Holds an array of wikilinks to items that must be completed first.
...
**format — script:** youtube | short | audio | video | podcast | lecture
**format — workshop:** workshop | lecture
...
Maps to: `log: habit` items.

**finance_entries**
Stores individual revenue and expense entries.
Maps to: `log: finance` items.

On export both companion tables reconstruct into markdown tables in the item body,
...
**Has resources field:**
- All creation subtypes
- reference: principle, directive, guide
- log: goal
- review: all subtypes
- journal: devlog
- action: project

**Does not have resources field:**
- reference: slip, literature, source, quote, identity, offer, asset, software, course
- log: habit, finance, contact, outreach
- action: task
- journal: daily, istikarah, dream, scratch
- inbox
...
### Creation — transmission merge
Workshop, script, podcast, and lecture were previously a separate transmission type.
They are unified under creation since all are items you author and develop.
Use the `format` field to distinguish delivery context.

### Comic and Manuscript
A comic series uses `creation: manuscript` as its container.
Individual issues use `creation: comic`.
Issues link back via the `series` field.
This mirrors the chapter/manuscript relationship exactly.

### Poetry and Story collections
A poetry collection uses `creation: manuscript` with `medium: poetry`.
Individual poems link back via the `series` field.
A short story anthology uses `creation: manuscript` with `medium: short-stories`.

### Course and Module
A course item is the container. Individual modules link back via `course: "[[]]"`.
...
### Companion tables
`habit_logs` and `finance_entries` store individual log entries separately.
On markdown export they reconstruct into body tables automatically.
...
Habit tracking lives in `log: habit` items — not here.
`1% better tomorrow` prompt appears only in this subtype.
...
**rating:** 1–5 numeric scale. Quick daily score queryable across days.
...
Always private. Signs section grows over time as guidance manifests.
...
One item per session. Links back to its parent project and series.
...
**Part 2 — Creation & Reference:**
## creation

Your intellectual, artistic, and delivery output.
The work itself — essays, stories, art, books, workshops, scripts.
Lives in the vault as items developed over time.
...
Your intellectual property. Used in coaching, content, and workshops.
...
The container item — individual chapters link back to this.
Also used as the container for comic series and poetry collections.
...
Panels are written inline in the script section.
No separate panel items needed.
...
A structured teaching or facilitation session delivered to an audience.
Use `format` to distinguish between an interactive workshop and a formal lecture.
...
**format valid values:** workshop | lecture

---

### script

Written script for video, audio, or live content.
Use `format` to distinguish the delivery medium.
Podcast episodes use this subtype with `format: podcast`.
...
**format valid values:** youtube | short | audio | video | podcast | lecture
...
A small collection of stable reference items about your inner world.
...
No resources field — source is the origin, not a synthesis item.
...
Reusable operational items for your business.
...
A finished software product item.
...
The container item — individual modules link back to this.
...
**Part 3 — Log, Review, Action & Inbox:**
## log
...
Used to measure lead generation activity against 12 week year goals.
...
Structured reflection items tied to a time period or life area.
Where you assess progress, extract lessons, and set direction.
`1% Better` prompt lives only in `journal: daily` — not in any review subtype.
...
A life area item. One of your 12 areas of life.
...
Tasks link back to this item via their `project` field.
...
*End of schema reference. Version 3.0 — 2026-04-02*

There it is. Copy all three parts into a single file called schema-reference.md. You now have both documents ready to hand to Codex.
